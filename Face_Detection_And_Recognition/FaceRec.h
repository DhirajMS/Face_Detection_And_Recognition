...
void detectAndDisplay(Mat frame)
{
...

	{
		...
		stringstream ssfn;
		filename = "G:\\Work\\FD_C++\\Face_Detection_And_Recognition\\x64\\Debug\\Faces\\";
...
void addFace()
{
	...
	if (!face_cascade.load("C:\\opencv\\etc\\haarcascades\\haarcascade_frontalface_alt.xml"))
	{
		cout << "error" << endl;
		return;
	};
...
void eigenFaceTrainer() {
	...
	model->save("G:\\Work\\FD_C++\\Face_Detection_And_Recognition\\x64\\Debug\\eigenface.yml");
...
void  FaceRecognition() {
	...
	model->read("G:\\Work\\FD_C++\\Face_Detection_And_Recognition\\x64\\Debug\\eigenface.yml");

	Mat testSample = imread("G:\\Work\\FD_C++\\Face_Detection_And_Recognition\\x64\\Debug\\0.jpg", 0);
	...
	while (true)
	{
		...
		if (!frame.empty()) {
			...
			if( faces.size() != 0)
			putText(original, "Number of Persons detected: " + to_string(faces.size()), Point(30, 90), FONT_HERSHEY_COMPLEX_SMALL, 1.0, CV_RGB(0, 255, 0), 1.0);
			else
			putText(original, "No Face Detected ", Point(30, 90), FONT_HERSHEY_COMPLEX_SMALL, 1.0, CV_RGB(0, 255, 0), 1.0);
